...
The generated files include a fingerprint and the application module for angularJS includes the .run file.
Use this command to generate and run an instance of the application in standalone mode, it means without the ps app application.

gulp dist -s
To build the application in dist folder. The generated files don't include a fingerprint and the application module for angular does not include the .run file.
This command should be executed to generate a the proper files to be used in the ps app application.
The parameter (-s) is necessary to run tasks for distribution.
...
gulp inject:less
To inject sass references in index.less
